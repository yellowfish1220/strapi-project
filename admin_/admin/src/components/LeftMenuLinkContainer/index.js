...
function LeftMenuLinkContainer({ layout, plugins, hidePluginUsersPermissions }) {
  ...
  const pluginsLinks = !isEmpty(pluginsObject) ? (
    map(sortBy(pluginsObject, 'name'), plugin => {
      if (plugin.id !== 'email' && plugin.id !== 'settings-manager') {
        ...
        const destination =
          plugin.id === 'content-manager' ? `${basePath}/ctm-configurations` : basePath;

        // Remove the `users-permissions` plugin if needed
        if (plugin.id === 'users-permissions' && hidePluginUsersPermissions) {
          return null;
        }
...
LeftMenuLinkContainer.propTypes = {
  hidePluginUsersPermissions: PropTypes.bool.isRequired,
